...
% Define the number of files (in your case 24 files)
num_files = 24;
...
for j = 1:1:12   % For the first part of the filename
    for i = 1:1:2   % For the second part of the filename
        ...
        for epoch = 1:num_epochs
            ...
            epoch_data = eeg_data(start_idx:end_idx, 2:17);  %% Select the column from 2 to 17 as first column is time 
...
for epoch_index = 1:num_epochs_total
    y = epoch_data_all{epoch_index}; 
    
    % Extract the numeric data from the table
    y_numeric = table2array(y);  % Convert the table to a numeric array
    
    % Apply the bandpass filter for all channels
    for ch = 1:1:16
        % Pass the numeric data for the current channel to the bpf_filter function
        out(ch,:) = bpf_filter(y_numeric(:,ch), fs);  
    end
    
    % Initialize an array to store the results for this epoch
    results = zeros(1, 771);  % 16 channels x 48 (12x4)features + 2 for AW and Valence indices+label column
    ...
    % Store the AW and Valence indices in the last two columns
...
writematrix(final_results, 'metrics_values_bpf.csv');


%For xtest, xtrain, ytest, ytrain

data=importdata("metrics_values_bpf.csv");
x=data(:,1:(end-1));
y=data(:,end);
[row,col]=size(x);
